...
const APP_VERSION = '73.4.0';
const SERVICE_NAME = 'newtaipei-noise-control-system-v73-richmenu-alias';
...
const LINE_RICHMENU_ALIAS_FIELD = env('LINE_RICHMENU_ALIAS_FIELD', 'ntpc-field');
const LINE_RICHMENU_ALIAS_EQUIPMENT = env('LINE_RICHMENU_ALIAS_EQUIPMENT', 'ntpc-equipment');
const LINE_RICHMENU_ALIAS_RESULTS = env('LINE_RICHMENU_ALIAS_RESULTS', 'ntpc-results');
...
function liveFieldFromRows(rows) {
  ...
  for (const r of rows || []) {
    ...
    if (!id) continue;
    const dateRaw = firstNonEmpty(r, ['日期','架設日期','架設日','執勤日期','外勤回報場次日','場次日']);
    const submitRaw = firstNonEmpty(r, ['送出時間','回報時間','更新時間','時間戳記']);
    const postRaw = firstNonEmpty(r, ['後校正日','後校正日期','收設備後校正日','收設備日期','撤收日期']);
    const d1 = parseRocDate(dateRaw);
    const d2 = parseRocDate(submitRaw);
    const p1 = parseRocDate(postRaw);
    const ts = Math.max(d1 ? d1.getTime() : 0, d2 ? d2.getTime() : 0);
    const postTs = p1 ? p1.getTime() : 0;
    if (postTs) {
      const currentPost = latestPost.get(id);
      if (!currentPost || postTs >= currentPost.postCalibrationTs) {
        latestPost.set(id, {
          deviceId: id,
          postCalibrationDate: rocDateText(postRaw),
          postCalibrationAgeDays: dayAgeFromDate(p1),
          postCalibrationTs: postTs,
          source: 'field_report',
    ...
    const item = {
      ...
      latestSessionHasPostCalibration: !!postTs,
      latestSessionPostCalibrationDate: postTs ? rocDateText(postRaw) : '',
      latestSessionPostCalibrationTs: postTs,
      district: r['行政區'] || '-',
...
function compareEquipmentLiveState(hit, post) {
  ...
  const latestSessionClosed = !!(hit && hit.latestSessionHasPostCalibration);

  // 設備生命週期判定：以「最新架設場次」為主。
  // 1) 最新架設場次未填後校正日：代表設備仍在該點監測，判定運作中。
  // 2) 最新架設場次已填後校正日：代表設備已收回，若沒有下一筆新場次，判定維護中。
  // 3) 若後校正日之後已有更新的架設場次：代表移動至下一點，重新判定運作中。
  if (!hasInstall) {
    return { state:'維護中', light:'⚫', stateLabel:'維護中', reason: hasPost ? '有後校正日，但尚無外勤回報新增場次' : '無架設回報、無後校正資料', maintenance:true };
  }

  if (latestSessionClosed) {
    return { state:'維護中', light:'⚫', stateLabel:'維護中', reason:'最新架設場次已填後校正日，表示設備已收回，尚未新增下一場', maintenance:true };
  }

  if (hasPost && post.postCalibrationTs > hit.onlineStartTs) {
    return { state:'維護中', light:'⚫', stateLabel:'維護中', reason:'後校正日晚於最新架設場次日，表示設備已收回，尚未新增下一場', maintenance:true };
  }

  return { state:'運作中', light:'🟢', stateLabel:'運作中', reason: hasPost ? '後校正後已有新增架設場次，且最新場次尚未填後校正日' : '最新架設場次未填後校正日', online:true };
}
function liveEquipmentFromRows(rows, store = getStore()) {
  const latestMap = liveFieldFromRows(rows || []);
  const fieldPostMap = latestMap.latestPost || new Map();
  const equipmentPostMap = equipmentPostCalibrationMap(store);
  const ids = knownEquipmentIds(store, latestMap);
  return ids.map(id => {
    const hit = latestMap.get(id) || null;
    const fieldPost = fieldPostMap.get(id) || null;
    const equipmentPost = equipmentPostMap.get(id) || null;
    const post = [fieldPost, equipmentPost].filter(Boolean).sort((a,b)=>(b.postCalibrationTs||0)-(a.postCalibrationTs||0))[0] || null;
    ...
    const base = hit || {
      ...
      onlineStartTs:0,
      district:'-',
      location: post ? '尚無新增外勤回報場次' : '尚無外勤回報場次',
      speedLimit:'-',
      noiseStandard:'-',
      calibration:'-',
      signDistance:'-',
...
function liveEquipmentText() {
  ...
  const header = `【即時機況】\n🟢運作中 ${counts.online}｜⚫維護中 ${counts.maintenance}`;
...
function liveEquipmentCardsHtml(list) {
  ...
  return items.map(x => {
    ...
    const stateText = online ? '運作中' : '維護中';
    const icon = online ? '🟢' : '⚫';
...
function lineReplyFor(msg, source = null) {
  ...
  if (/^(外勤回報|外勤平台|填寫回報|我要回報|填外勤|外勤表單|新增回報|照片上傳|座標定位|回報彙整|草稿暫存)$/.test(m)) return platformButtonMessage('field');
  ...
  if (/即時機況|目前機況|今天機況|現場機況|機台機況|設備機況|機台狀態|設備狀態|設備即時|設備列表|上線監測|在線|線上|哪台在線|哪些機台|機台在哪|設備在哪/.test(m)) return withQuick(liveEquipmentText(), ['設備管理','即時機況','外勤回報','進度']);
  if (/設備到期|到期提醒|快到期|過期|比測|噪音計|風速計|校正|檢定|設備管理|設備查詢|後校正記錄|校正管理|維護記錄|設定管理|設備日誌|查.*設備|看.*設備/.test(m)) return withQuick(equipmentText(), ['即時機況','設備管理','進度','法規中心']);

  if (/百大|熱點|排名|第\d+名|前十|top10|Top10|布點建議|建議點位/.test(m)) return withQuick(hotspotsText(m), [...QUICK_DISTRICTS, '百大公式']);

  if (/^(進度|計畫進度|成果摘要|統計查詢|成果統計|圖表視覺化|設定篩選|目前成果|今年成果|年度成果)$/.test(q) || /看.*(進度|成果|統計)|查.*(進度|成果|統計)|現在.*(進度|成果)/.test(q)) return withQuick(progressText(), [...QUICK_MONTHS, ...QUICK_DISTRICTS.slice(0,4), '案件追蹤']);
...
function equipmentLivePage() {
  const data = liveEquipmentStatusData();
  const rows = (data.equipmentLive || []).map(e => `
    <tr>
      <td>${serverEscHtml(e.deviceId || '-')}</td>
      <td>${serverEscHtml(e.stateLabel || e.state || '-')}</td>
      <td>${serverEscHtml(e.sessionNo || '-')}</td>
      <td>${serverEscHtml(e.onlineDate || '-')}</td>
      <td>${serverEscHtml(e.postCalibrationDate || '-')}</td>
      <td>${serverEscHtml(e.maintenanceReason || e.reason || '-')}</td>
    </tr>`).join('');
  return `<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>即時機況</title><style>${css()}</style></head><body><main>
  <div class="top"><h1>即時機況巡檢</h1><a class="btn" href="/admin.html">返回後台</a></div>
  <section class="panel"><p class="eyebrow small">LIVE EQUIPMENT</p><h2>整體狀態：${serverEscHtml(data.summary?.overallState || data.overallState || '-')}</h2><p>更新時間：${serverEscHtml(data.updatedAtTaipei || data.updatedAt || taipeiTime())}</p>
  <div class="quick-tags"><span>運作中 ${serverEscHtml(data.summary?.online ?? 0)}</span><span>維護中 ${serverEscHtml(data.summary?.maintenance ?? 0)}</span></div></section>
  <section class="panel"><h2>設備明細</h2><table><thead><tr><th>設備</th><th>狀態</th><th>最近場次</th><th>最近架設</th><th>後校正日</th><th>說明</th></tr></thead><tbody>${rows || '<tr><td colspan="6">尚未同步外勤或設備資料。</td></tr>'}</tbody></table></section>
  </main></body></html>`;
}
...
function liveEquipmentCards(items){return (items||[]).map(e=>{const st=String(e.state||''); const online=st.includes('運作')||st.includes('上線'); const cls=online?'ok':'bad'; const icon=online?'🟢':'⚫'; const label=online?'運作中':'維護中'; return '<div class="module-card '+cls+'"><span class="icon">'+icon+'</span><b>'+escHtml(e.deviceId||'-')+'</b><small>'+escHtml(e.state||'-')+'<br>架設：'+escHtml(e.onlineDate||'-')+'｜場次：'+escHtml(e.sessionNo||'-')+'<br>後校正：'+escHtml(e.postCalibrationDate||'-')+'<br>'+escHtml(e.district||'-')+'｜'+escHtml(e.location||'-')+'<br>'+escHtml(e.maintenanceReason||'')+'</small><span class="state">'+label+'</span></div>';}).join('') || '<div class="loading-line">尚未同步外勤機況。</div>'}
...
function pageAlias(page) {
  return {
    main: LINE_RICHMENU_ALIAS_MAIN,
    field: LINE_RICHMENU_ALIAS_FIELD,
    equipment: LINE_RICHMENU_ALIAS_EQUIPMENT,
    results: LINE_RICHMENU_ALIAS_RESULTS
  }[page] || LINE_RICHMENU_ALIAS_MAIN;
}
function switchTo(page, data = '') {
  return richMenuSwitchAction(pageAlias(page), data || `switch:${page}`);
}
function lineRichMenuPayload(page = 'main') {
  // V73.4 四頁 Rich Menu：採 Rich Menu Alias + Switch Action 分頁切換。
  // 每頁固定使用 LINE 官方大尺寸 2500x1686，確保顯示比例一致。
  const w = 2500, h = 1686;
  const common = {
  ...
  const prevNext = (prev, next) => [
    area(2120, 1545, 185, 95, switchTo(prev, `prev:${prev}`)),
    area(2310, 1545, 185, 95, switchTo(next, `next:${next}`))
  ];
  const mainButtons = [
    area(80, 610, 520, 350, switchTo('field', 'open:field')),
    area(660, 610, 520, 350, switchTo('results', 'open:results')),
    area(1240, 610, 520, 350, textAction('即時機況')),
    area(1820, 610, 520, 350, uriAction(HOTSPOT_URL)),
    area(80, 1010, 520, 350, switchTo('equipment', 'open:equipment')),
    area(660, 1010, 520, 350, uriAction(CASE_TRACKING_URL)),
    area(1240, 1010, 520, 350, textAction('月報統計')),
    area(1820, 1010, 520, 350, textAction('新聞')),
    area(2350, 1530, 145, 150, textAction(ADMIN_HIDDEN_TAP_TOKEN))
  ];
  const fieldButtons = [
    area(80, 610, 520, 350, uriAction(FIELD_REPORT_URL)),
    area(660, 610, 520, 350, uriAction(FIELD_REPORT_URL)),
    area(1240, 610, 520, 350, textAction('座標定位')),
    area(1820, 610, 520, 350, uriAction(FIELD_REPORT_URL)),
    area(80, 1010, 520, 350, uriAction(FIELD_REPORT_URL)),
    area(660, 1010, 520, 350, switchTo('main', 'back:main')),
    ...prevNext('main', 'equipment')
  ];
  const equipmentButtons = [
    area(80, 610, 520, 350, textAction('設備列表')),
    area(660, 610, 520, 350, textAction('即時機況')),
    area(1240, 610, 520, 350, textAction('後校正記錄')),
    area(1820, 610, 520, 350, textAction('校正管理')),
    area(80, 1010, 520, 350, textAction('維護記錄')),
    area(660, 1010, 520, 350, textAction('設定管理')),
    area(1240, 1010, 520, 350, textAction('耗材管理')),
    area(1820, 1010, 520, 350, switchTo('main', 'back:main')),
    ...prevNext('field', 'results')
  ];
  const resultsButtons = [
    area(80, 610, 520, 350, uriAction(DASHBOARD_URL)),
    area(660, 610, 520, 350, textAction('行政區查詢')),
    area(1240, 610, 520, 350, textAction('趨勢分析')),
    area(1820, 610, 520, 350, uriAction(DASHBOARD_URL)),
    area(80, 1010, 520, 350, uriAction(CASE_TRACKING_URL)),
    area(660, 1010, 520, 350, uriAction(DASHBOARD_URL)),
    area(1240, 1010, 520, 350, textAction('時段分析')),
    area(1820, 1010, 520, 350, switchTo('main', 'back:main')),
    ...prevNext('equipment', 'main')
  ];
  const map = {
    main: { name: 'NTPC Noise V73.4 Main Rich Menu', areas: mainButtons },
    field: { name: 'NTPC Noise V73.4 Field Rich Menu', areas: fieldButtons },
    equipment: { name: 'NTPC Noise V73.4 Equipment Rich Menu', areas: equipmentButtons },
    results: { name: 'NTPC Noise V73.4 Results Rich Menu', areas: resultsButtons }
  };
  return { ...common, ...(map[page] || map.main) };
}
...
async function cleanupOldNtpcRichMenus(keepRichMenuIds) {
  const keep = new Set(Array.isArray(keepRichMenuIds) ? keepRichMenuIds : [keepRichMenuIds]);
  ...
  if (before.ok && before.data && Array.isArray(before.data.richmenus)) {
    for (const m of before.data.richmenus) {
      ...
      if (id && !keep.has(id) && /^NTPC Noise/i.test(name)) {
...
async function createUploadAndAliasRichMenu(page, imageFile, aliasId) {
  const imagePath = path.join(PUBLIC_DIR, 'assets', imageFile);
  const imageStat = fs.existsSync(imagePath) ? fs.statSync(imagePath) : null;
  const create = await lineApiJson('/v2/bot/richmenu', 'POST', lineRichMenuPayload(page));
  if (!create.ok) return { ok:false, page, aliasId, step:'create', create, imageFile };
  const richMenuId = create.data.richMenuId;
  const upload = await uploadRichMenuImage(richMenuId, imagePath);
  if (!upload.ok) return { ok:false, page, aliasId, step:'upload', richMenuId, upload, imageFile, imageBytes:imageStat?.size ?? null };
  const alias = await upsertRichMenuAlias(aliasId, richMenuId);
  if (!alias.ok) return { ok:false, page, aliasId, step:'alias', richMenuId, upload, alias, imageFile, imageBytes:imageStat?.size ?? null };
  return { ok:true, page, aliasId, richMenuId, imageFile, imageBytes:imageStat?.size ?? null, create, upload, alias };
}
...
async function updateRichMenu() {
  ...
  const pages = [
    { page:'main', imageFile:'line-rich-menu-main.jpg', aliasId:LINE_RICHMENU_ALIAS_MAIN },
    { page:'field', imageFile:'line-rich-menu-field.jpg', aliasId:LINE_RICHMENU_ALIAS_FIELD },
    { page:'equipment', imageFile:'line-rich-menu-equipment.jpg', aliasId:LINE_RICHMENU_ALIAS_EQUIPMENT },
    { page:'results', imageFile:'line-rich-menu-results.jpg', aliasId:LINE_RICHMENU_ALIAS_RESULTS }
  ];
  ...
  for (const cfg of pages) {
    const r = await createUploadAndAliasRichMenu(cfg.page, cfg.imageFile, cfg.aliasId);
    created.push(r);
    if (!r.ok) return { ok:false, step:`${cfg.page}:${r.step}`, created, richMenusBefore, defaultBefore };
  }

  const main = created.find(x => x.page === 'main');
  const unsetDefault = await deleteDefaultRichMenuSafe();
  const setDefault = await lineApiJson(`/v2/bot/user/all/richmenu/${main.richMenuId}`, 'POST');
  if (!setDefault.ok) return { ok:false, step:'set default rich menu', created, unsetDefault, setDefault, richMenusBefore, defaultBefore };

  const cleanup = await cleanupOldNtpcRichMenus(created.map(x => x.richMenuId));
  const defaultAfter = await getDefaultRichMenu();

  return {
    ok: !!setDefault.ok,
    mode: 'multi-page-rich-menu-v73.4-alias-switch-action',
    aliases: {
      main: LINE_RICHMENU_ALIAS_MAIN,
      field: LINE_RICHMENU_ALIAS_FIELD,
      equipment: LINE_RICHMENU_ALIAS_EQUIPMENT,
    ...
    richMenus: created.map(x => ({ page:x.page, aliasId:x.aliasId, richMenuId:x.richMenuId, imageFile:x.imageFile, imageBytes:x.imageBytes, upload:x.upload, alias:x.alias })),
    unsetDefault,
    setDefault,
    defaultBefore,
    defaultAfter,
...
function systemStatusSnapshot(lineStatus = null) {
  ...
  const modules = [
    ...
    { key:'richmenu', name:'Rich Menu', icon:'☰', ok: richMenuReady, status: richMenuReady ? '已套用' : '待更新', desc:'四頁式：主選單、外勤回報、設備管理、成果查詢 Alias 切換' },
    ...
    { key:'liveDevice', name:'即時機況', icon:'📡', ok: Array.isArray(store.equipmentLive) && store.equipmentLive.length > 0 ? true : null, status: (Array.isArray(store.equipmentLive) && store.equipmentLive.length > 0) ? `${liveEquipmentCounts(store.equipmentLive).online} 運作｜${liveEquipmentCounts(store.equipmentLive).maintenance} 維護` : '待同步', desc:'外勤回報最新場次日＋後校正日推算設備生命週期；最新場次未填後校正日為上線' },
  ...
  return {
    ...
    richMenuReady,
    richMenuMode: 'V73.5 四頁式 Rich Menu Alias + Switch Action / 主選單、外勤回報、設備管理、成果查詢',
...
async function syncLiveEquipmentFromFieldSheet() {
  ...
  store.lastLiveEquipmentSync = { at:nowIso(), atTaipei:taipeiTime(), ok:true, count:items.length, counts, sheetId:env('FIELD_GOOGLE_SHEET_ID'), gid:FIELD_GOOGLE_SHEET_GID };
  safeJsonWrite(STORE_PATH, store);
  return { ok:true, checkedAt:store.lastLiveEquipmentSync.atTaipei, count:items.length, counts, equipmentLive:items, sheetId:env('FIELD_GOOGLE_SHEET_ID'), gid:FIELD_GOOGLE_SHEET_GID };
}
function liveEquipmentStatusData() {
  const store = getStore();
  const items = Array.isArray(store.equipmentLive) ? store.equipmentLive : [];
  const counts = liveEquipmentCounts(items);
  const summary = { ...counts, overallState: counts.maintenance > 0 ? '部分設備維護中' : (items.length ? '設備運作中' : '尚未同步') };
  return { ok:true, sheetId:env('FIELD_GOOGLE_SHEET_ID'), gid:FIELD_GOOGLE_SHEET_GID, sheetUrl:FIELD_GOOGLE_SHEET_URL, count:items.length, counts, summary, lastSync:store.lastLiveEquipmentSync || null, equipmentLive:items };
}
...
async function handle(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && pathname === '/equipment-live.html') return html(res, equipmentLivePage());
    if (req.method === 'GET' && pathname === '/api/admin/equipment-live/status') return json(res, 200, liveEquipmentStatusData());
    if (req.method === 'GET' && pathname === '/api/admin/equipment-live/sync-status') return json(res, 200, liveEquipmentStatusData());
