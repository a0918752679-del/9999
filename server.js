...
const SERVICE_NAME = 'newtaipei-noise-control-system-v83-richmenu-only-patch';
...
const LINE_RICHMENU_ALIAS_ADMIN = env('LINE_RICHMENU_ALIAS_ADMIN', 'ntpc-admin');
...
function lineRichMenuPayload(page = 'field') {
  // V83：只更新 LINE Rich Menu，不改原有後台/LINE Bot 主功能。
  // 使用者指定三張圖：外勤作業、行政作業、相關資訊。
  const configs = {
    field: {
      size: { width: 1536, height: 768 },
      name: 'NTPC Noise V83 Field Rich Menu',
      areas: [
        // 頂部頁籤 / 切換區
        area(0, 0, 250, 145, richMenuSwitchAction(LINE_RICHMENU_ALIAS_FIELD, 'switch-to-field')),
        area(1040, 0, 250, 145, richMenuSwitchAction(LINE_RICHMENU_ALIAS_ADMIN, 'switch-to-admin')),
        area(1290, 0, 246, 145, richMenuSwitchAction(LINE_RICHMENU_ALIAS_INFO, 'switch-to-info')),
        // 功能卡
        area(35, 405, 480, 270, { type: 'uri', uri: FIELD_REPORT_URL }),
        area(520, 405, 480, 270, { type: 'uri', uri: `${FIELD_REPORT_URL}?mode=post-calibration` }),
        area(1005, 405, 495, 270, { type: 'message', text: '即時機況' }),
        // 隱藏後台入口：避免破壞畫面，保留右下角連點三下機制
        area(1420, 690, 110, 70, { type: 'message', text: ADMIN_HIDDEN_TAP_TOKEN })
      ]
    },
    admin: {
      size: { width: 1536, height: 793 },
      name: 'NTPC Noise V83 Admin Rich Menu',
      areas: [
        area(985, 0, 260, 140, richMenuSwitchAction(LINE_RICHMENU_ALIAS_FIELD, 'switch-to-field')),
        area(0, 0, 250, 145, richMenuSwitchAction(LINE_RICHMENU_ALIAS_ADMIN, 'switch-to-admin')),
        area(1250, 0, 286, 140, richMenuSwitchAction(LINE_RICHMENU_ALIAS_INFO, 'switch-to-info')),
        area(30, 450, 390, 250, { type: 'uri', uri: `${FIELD_REPORT_URL}/summary` }),
        area(425, 450, 390, 250, { type: 'message', text: '設備管理' }),
        area(820, 450, 390, 250, { type: 'uri', uri: CASE_TRACKING_URL }),
        area(1215, 450, 300, 250, { type: 'message', text: '資料安全' }),
        area(1420, 710, 110, 70, { type: 'message', text: ADMIN_HIDDEN_TAP_TOKEN })
      ]
    },
    info: {
      size: { width: 1536, height: 864 },
      name: 'NTPC Noise V83 Info Rich Menu',
      areas: [
        area(1010, 0, 250, 145, richMenuSwitchAction(LINE_RICHMENU_ALIAS_FIELD, 'switch-to-field')),
        area(1260, 0, 276, 145, richMenuSwitchAction(LINE_RICHMENU_ALIAS_ADMIN, 'switch-to-admin')),
        area(0, 0, 250, 145, richMenuSwitchAction(LINE_RICHMENU_ALIAS_INFO, 'switch-to-info')),
        area(35, 500, 470, 245, { type: 'message', text: 'Google Sheet 連結' }),
        area(515, 500, 470, 245, { type: 'uri', uri: `${PUBLIC_BASE_URL}/line-help.html` }),
        area(995, 500, 505, 245, { type: 'message', text: '法規中心' }),
        area(1420, 790, 110, 70, { type: 'message', text: ADMIN_HIDDEN_TAP_TOKEN })
      ]
    }
  };
  const cfg = configs[page] || configs.field;
  return {
    size: cfg.size,
    selected: true,
    name: cfg.name,
...
async function cleanupOldNtpcRichMenus(keepRichMenuIds) {
  const keepSet = new Set(Array.isArray(keepRichMenuIds) ? keepRichMenuIds : [keepRichMenuIds].filter(Boolean));
  ...
  if (before.ok && before.data && Array.isArray(before.data.richmenus)) {
    for (const m of before.data.richmenus) {
      ...
      if (id && !keepSet.has(id) && /^NTPC Noise/i.test(name)) {
...
async function updateRichMenu() {
  ...
  const images = {
    field: path.join(PUBLIC_DIR, 'assets', 'line-richmenu-field.jpg'),
    admin: path.join(PUBLIC_DIR, 'assets', 'line-richmenu-admin.jpg'),
    info: path.join(PUBLIC_DIR, 'assets', 'line-richmenu-info.jpg')
  };
  const missingImages = Object.entries(images).filter(([,p]) => !fs.existsSync(p)).map(([k,p]) => ({ page:k, path:p }));
  if (missingImages.length) return { ok:false, step:'check rich menu images', missingImages };
  ...
  for (const page of ['field', 'admin', 'info']) {
    const create = await lineApiJson('/v2/bot/richmenu', 'POST', lineRichMenuPayload(page));
    if (!create.ok) return { ok:false, step:`create ${page} rich menu`, page, create, richMenusBefore, defaultBefore };
    created[page] = { ok:true, richMenuId:create.data.richMenuId, create };

    const upload = await uploadRichMenuImage(create.data.richMenuId, images[page]);
    if (!upload.ok) return { ok:false, step:`upload ${page} rich menu image`, page, richMenuId:create.data.richMenuId, upload, created, richMenusBefore, defaultBefore };
    uploaded[page] = upload;
  }

  aliases.field = await upsertRichMenuAlias(LINE_RICHMENU_ALIAS_FIELD, created.field.richMenuId);
  if (!aliases.field.ok) return { ok:false, step:'upsert field alias', aliases, created, uploaded };
  aliases.admin = await upsertRichMenuAlias(LINE_RICHMENU_ALIAS_ADMIN, created.admin.richMenuId);
  if (!aliases.admin.ok) return { ok:false, step:'upsert admin alias', aliases, created, uploaded };
  aliases.info = await upsertRichMenuAlias(LINE_RICHMENU_ALIAS_INFO, created.info.richMenuId);
  if (!aliases.info.ok) return { ok:false, step:'upsert info alias', aliases, created, uploaded };

  const unsetDefault = await deleteDefaultRichMenuSafe();
  const setDefault = await lineApiJson(`/v2/bot/user/all/richmenu/${created.field.richMenuId}`, 'POST');
  if (!setDefault.ok) return { ok:false, step:'set default field rich menu', created, uploaded, aliases, unsetDefault, setDefault, richMenusBefore, defaultBefore };

  const cleanup = await cleanupOldNtpcRichMenus([created.field.richMenuId, created.admin.richMenuId, created.info.richMenuId]);
  const defaultAfter = await getDefaultRichMenu();

  return {
    ok: true,
    mode: 'v83-three-tab-richmenu-only-patch-user-selected-images',
    defaultPage: 'field',
    aliases: {
      field: { aliasId: LINE_RICHMENU_ALIAS_FIELD, ...aliases.field },
      admin: { aliasId: LINE_RICHMENU_ALIAS_ADMIN, ...aliases.admin },
      info: { aliasId: LINE_RICHMENU_ALIAS_INFO, ...aliases.info }
    },
    richMenuIds: {
      field: created.field.richMenuId,
      admin: created.admin.richMenuId,
      info: created.info.richMenuId
    },
    images: Object.fromEntries(Object.entries(images).map(([k,p]) => [k, { file:path.basename(p), bytes:fs.statSync(p).size }])),
    created,
    uploaded,
    unsetDefault,
    setDefault,
...
function systemStatusSnapshot(lineStatus = null) {
  ...
  const modules = [
    ...
    { key:'richmenu', name:'Rich Menu', icon:'☰', ok: richMenuReady, status: richMenuReady ? '已套用' : '待更新', desc:'三頁籤：外勤作業、行政作業、相關資訊切換' },
  ...
  return {
    ...
    richMenuReady,
    richMenuMode: 'V83 三頁籤 Rich Menu / 三張使用者指定圖 / richmenuswitch 切換 / Sheet 安全驗證',
...
async function handle(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && pathname === '/api/admin/line/rich-menu-diagnostics') return json(res, 200, { ok:true, serviceName:SERVICE_NAME, appVersion:APP_VERSION, defaultRichMenu: await getDefaultRichMenu(), richMenus: await listRichMenus(), localImages: ['line-richmenu-field.jpg','line-richmenu-admin.jpg','line-richmenu-info.jpg'].map(file => { const p=path.join(PUBLIC_DIR,'assets',file); return fs.existsSync(p) ? { file, bytes:fs.statSync(p).size } : { file, missing:true }; }), aliases:{ field:LINE_RICHMENU_ALIAS_FIELD, admin:LINE_RICHMENU_ALIAS_ADMIN, info:LINE_RICHMENU_ALIAS_INFO } });
